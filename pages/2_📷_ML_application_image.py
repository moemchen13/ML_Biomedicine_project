...
def train_model(data,augmented_data,n_epochs=100,use_augmented_data=False):
    data_to_train = data
    ...
    model.fit(X_train/255, y_train, epochs=n_epochs, batch_size=32,callbacks=[early_stop],verbose=1,validation_split=0.2)

    _,model_acc,auc = model.evaluate(data[1]/255,data[3])
    _,model_acc_aug,_ = model.evaluate(augmented_data[1]/255,augmented_data[3])
...
def audio_button(label,file="./music/wii_music.mp3",loop=True, start=0):
    """
...
if f:
    ...
    with middle:
        ...
        images=[X_train[0,:,:,:],X_train[1,:,:,:],X_train[2,:,:,:],X_train[3,:,:,:],X_train[4,:,:,:],X_train[5,:,:,:],X_train[6,:,:,:],X_train[7,:,:,:]],
...
left,rest =st.columns([0.2,0.8])
with left:
    n_epochs = st.number_input("Number of Epochs to run:",min_value=30,max_value=1000,step=10,value=100)
if audio_button("Train models",file="./music/wii_music.mp3"):

    model = create_model(augmented_data[0].shape[0],augmented_data[0].shape[1],augmented_data[0].shape[2],len(np.unique(augmented_data[1])))
    model.summary(print_fn=lambda x: st.text(x))
    with st.spinner(text="In progress first model..."):
        
        model_without_augmentation,stats = train_model(data,augmented_data,n_epochs=n_epochs)
    ...
    with st.spinner(text="In progress second model..."):
        ...
        model_with_augmentation,stats_a = train_model(data,augmented_data,use_augmented_data=True,n_epochs=n_epochs)
...
left,middle = st.columns([0.25,0.75],vertical_alignment="bottom")
with left:
